...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo Y | ./config.sh --url https://dev.azure.com/wvdlabs --auth pat --token cdkm7pqubijt4xzjkxfqagwzybqz2yyrkhostnzlr6iwdnrt3hia --pool sha-ubuntu-01 --agent $(hostname) --replace",
      "sudo ./svc.sh install",
      "sudo ./svc.sh start"
    ]
  }
